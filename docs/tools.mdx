...
By default, omitted `needsApproval` behaves like `never()`, so tool calls may execute without human approval. Require approval or other safeguards for sensitive, irreversible, regulated, financial, healthcare, legal, user-impacting, or external side-effecting actions.
