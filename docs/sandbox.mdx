...
With the Vercel backend, `vercel()` resolves authorization strategies for every runtime step and passes the results to a function-form policy:
...
The strategy uses the same authorization contract as connections and authored tools. It may be a static `getToken` strategy, a per-user strategy with `principalType: "user"`, or a full interactive strategy such as Vercel Connect:
...
export default defineSandbox({
  backend: vercel({
    credentials: {
      service: { ...connect("example-service"), displayName: "Example Service" },
    },
    networkPolicy: ({ service }) => ({
      allow: {
        "api.example.com": [
...
When an interactive `getToken` reports that authorization is required, eve keeps the already-created sandbox on the empty-token policy, emits the normal `authorization.required` challenge, and durably parks the calling tool. After the callback completes, the tool is replayed, eve reattaches to the same sandbox, resolves the credential, and replaces the policy on the live sandbox before returning it to authored code.

Eve resolves credentials when authored code first accesses the sandbox during a step, applies the resulting policy, and replaces every brokered token with an empty value when the step ends or fails. If a non-interactive credential fails to resolve, that credential is represented by an empty token while the authored egress restrictions remain active. A failed cleanup fails the step rather than being ignored. Brokered credentials are unavailable during template bootstrap.
